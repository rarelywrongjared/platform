...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  ...
  <?php endif; ?>
  <?php print render($content['field_block_image_full']); ?>
  <?php if(!empty($user_picture) || $display_submitted): ?>
    <footer<?php print $footer_attributes; ?>>
      <div class="datetime"><?php print date("j F Y", $node->created); ?></div>
      <div class="author-datetime"><?php print ' by ' . $name?></div>
      <?php
        $comment_count = $content['comments']['#node']->comment_count;
        $comment_title = $comment_count == 1 ? ' Comment' : ' Comments';
      ?>
      <div class="comment"> <span><?php print ($comment_count . $comment_title) ?></span></div>
  ...
  <?php
    $block = module_invoke('views', 'block_view', 'author-block');
